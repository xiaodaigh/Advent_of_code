...
using DataFrames
df = DataFrame(y = collect(23:23:nmarbles))
df[:mody] = mod.(df.y, nplayers)

he(y) = mean(setdiff(1:y-1, 23:23:nmarbles))
df[:meanval] = he.(df.y)

df[:y_mean] = df.y + df.meanval

dfsumm= by(df, :mody, :y_mean => sum)

dfsumm.y_mean_sum |> maximum




function day9_3(nplayers, nmarbles)
